...
if ($_SESSION['user_type'] == 1 || $_SESSION['user_type'] == 2) {
    $done = mysqli_query($db_conn, 'SELECT * FROM f23_Job_T WHERE job_status = 1');
    $paused = mysqli_query($db_conn, 'SELECT * FROM f23_Job_T WHERE job_status = 3');
    $active = mysqli_query($db_conn, 'SELECT * FROM f23_Job_T WHERE job_status = 2');
} else {
    // view all jobs where the SESSION user is job_owner
    $done = mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 1 AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
    $paused = mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 3 AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
    $active = mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 2 AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
}
...
<body>
    ...
    <div class="w3-container w3-white w3-round w3-border w3-margin-bottom w3-margin-right" style="font-size:small">
        <header class="w3-round w3-container w3-blue w3-text-white" style="margin-left:-17px; margin-right:-17px">
            <h6> Active</h6>
        </header>
                    ...
                    <tr>
                        ...
                        <td>
                            <a class="<?php if ($job->job_status == 3) {
                                            echo "w3-hide";
                                        } ?>" href="./dashboard.php?content=jobslist&job_id=<?php echo $job->job_id; ?>&change_status=4"><button class="w3-btn w3-round w3-red">Archive</button></a>
                        </td>
                    </tr>
...
<div class="w3-container w3-white w3-round w3-border w3-margin-bottom w3-margin-right" style="font-size:small">
    ...
    <table class="w3-table w3-bordered w3-hoverable">
        ...
        <tbody>
            <!-- While Loop to Generate Rows of Tasks -->
            <?php while ($job = mysqli_fetch_object($done)) { ?>
                <tr>
                    <!-- Keep the same -->
                    <!--  <td><?php //echo $job->job_id; ?></td> -->
                    <!-- Get title -->
                    ...
                    <td class="<?php if ($admin_form_disabled == "disabled") {
                        ...
                        <a class=""><button onclick="document.getElementById('ad<?php echo $job->job_id; ?>').style.display='block'" class="w3-btn w3-round w3-blue">Add Steps</button></a>
                    </td>
                    <td>
                        <a class="<?php if ($job->job_status == 3) {
                                        echo "w3-hide";
                ...
                <div id="id<?php echo $job->job_id; ?>" class="w3-modal">
                    <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:600px; margin-top:-100px;">
...
<div class="w3-container w3-white w3-round w3-border w3-margin-bottom w3-margin-right" style="font-size:small">
    <header class="w3-round w3-container w3-orange w3-text-white" style="margin-left:-17px; margin-right:-17px">
        <h6> Paused</h6>
    </header>
    <table class="w3-table w3-bordered w3-hoverable">
        <thead>
        ...
        <tbody>
            <!-- While Loop to Generate Rows of Tasks -->
            <?php while ($job = mysqli_fetch_object($paused)) { ?>
                <tr>
                    <!-- Keep the same -->
                   <!--  <td><?php //echo $job->job_id; ?></td> -->
                    <!-- Get title -->
                    ...
                    <!-- Keep the same -->
                    <td>
                        <a class="<?php if ($job->job_status == 4) {
                                        echo "w3-hide";
                                    } ?>"><button onclick="document.getElementById('id<?php echo $job->job_id; ?>').style.display='block'" class="w3-btn w3-round w3-green">Details</button></a>
                    </td>
                    <td class="<?php if ($admin_form_disabled == "disabled") {
                ...
                <div id="id<?php echo $job->job_id; ?>" class="w3-modal">
                    <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:700px; margin-top:-100px;">
                        <header class="w3-round w3-container w3-orange w3-text-white">
                            <h6 class="w3-left"><?php echo $job->job_title; ?></h6>
                            <span onclick="document.getElementById('id<?php echo $job->job_id; ?>').style.display='none'" class="w3-button w3-display-topright w3-round">&times;</span>
                        </header>
                        <iframe height="700" width="700" style="border:none;" src="./userfunctions/jobs/view-one-job.php?job_id=<?php echo $job->job_id; ?>"></iframe>
