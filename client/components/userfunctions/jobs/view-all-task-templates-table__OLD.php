                            ...
                            <div id="id<?php echo $template->taskTemplate_id; ?>" class="w3-modal">
                                <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:600px; margin-top:-60px;">
                                    ...
                                    <div class="w3-container">
                                        <p><strong>Industry:</strong> <?php if ($template->ind_code == '0') echo 'None'; else echo $template->ind_code; ?></p>
                                        <p><strong>Type:</strong> <?php echo $template->task_type == 1 ? '<span class="w3-text-red"><b>Urgent</b></span>' : 'Normal'; ?></p>
                                        <p><strong>Status:</strong> 
                                            <?php 
                                            if ($template->templateStatus_id == 1) {
                                                echo '<span class="w3-text-green"><b>Ready</b></span>';
                                            } elseif ($template->templateStatus_id == 2) {
                                                echo '<span class="w3-text-orange"><b>Not Ready</b></span>';
                                            } else {
                                                echo 'Deleted';
                                            }
                                            ?>
                                        </p>
                                        <p><strong>Instructions:</strong> <?php echo $template->task_instructions; ?></p>
                                    ...
                                    <!-- End of existing information -->

                                    <!-- Display form instructions here -->
                                    <hr>
                                    <div class="w3-container">
                                        <?php
                                        // Retrieve form instructions based on $form_id
                                        $form_id = $template->form_id;
                                        if ($form_id != null && $form_id != 0) {
                                            $form_query = mysqli_query($db_conn, 'SELECT instructions FROM f23_Form_Templates WHERE TID = ' . $form_id);
                                            $form_result = mysqli_fetch_assoc($form_query);
                                            if ($form_result) {
                                                // Unpack and display form instructions
                                                $instructions_json = json_decode($form_result['instructions'], true);
                                                echo '<p><strong>Form Instructions:</strong></p>';
                                                foreach ($instructions_json as $instruction) {
                                                    echo '<p>' . $instruction . '</p>';
                                                }
                                            } else {
                                                echo '<p>No form instructions available.</p>';
                                            }
                                        } else {
                                            echo '<p>No form associated with this task template.</p>';
                                        }
                                        ?>
                                    </div>
                                    ...
                                    <!-- Display iframe or any other content you have -->
