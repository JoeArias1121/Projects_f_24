...
if ($_SESSION['user_type'] == 1) {
    ...
    $done =   mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,
j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T 
AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 1 
AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
    $paused = mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,
j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T 
AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 3 
AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
    $active = mysqli_query($db_conn, 'SELECT DISTINCT j.job_id,j.job_status,j.job_owner,
j.job_title,j.job_instructions,j.job_deadline,j.job_created FROM f23_Job_T AS j LEFT JOIN f23_JobDetails_T 
AS jd ON j.job_id = jd.job_id LEFT JOIN f23_StepDetails_T as td ON td.task_id = jd.task_id WHERE j.job_status = 2 
AND (td.user_id = '.$_SESSION['user_id'].' OR j.job_owner = '.$_SESSION['user_id'].')');
}
                ...
                <div class="w3-card w3-round-large w3-margin-bottom w3-hover-shadow">
                    ...
                    <footer class="">
                        <div class="w3-bar">
                            ...
                            <a href="./dashboard.php?content=jobs&job_id=<?php echo $job->job_id; ?>&change_status=4"><button class="w3-button w3-round ">Archive</button></a>
                        </div>
                    </footer>
                ...
                <div class="w3-card w3-round-large w3-margin-bottom w3-hover-shadow">
                    ...
                    <footer class="">
                        <div class="w3-bar">
                            <button class="w3-button w3-round w3-bar-item w3-padding <?php if ($job->job_status == 4) {
                                                                                            echo "w3-hide";
                                                                                        } ?>" onclick="document.getElementById('id<?php echo $job->job_id; ?>').style.display='block'">
                                See Details
                            </button>
                            <a href="./dashboard.php?content=jobs&job_id=<?php echo $job->job_id; ?>&change_status=4"><button class="w3-button w3-round">Archive</button></a>
                        </div>
                    ...
                    <div id="id<?php echo $job->job_id; ?>" class="w3-modal">
                        <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:700px; margin-top:-100px;">
                            <header class="w3-round w3-container w3-green">
                                <h6 class="w3-left">Job: <?php echo $job->job_title; ?></h6>
                                <span onclick="document.getElementById('id<?php echo $job->job_id; ?>').style.display='none'" class="w3-button w3-display-topright">&times;</span>
                            </header>
                            <iframe height="700" width="700" style="border:none;" src="./userfunctions/jobs/view-one-job.php?job_id=<?php echo $job->job_id; ?>"></iframe>
                        </div>
            ...
            <?php while ($job = mysqli_fetch_object($paused)) { ?>
                <div class="w3-card w3-round-large w3-margin-bottom w3-hover-shadow">
                    <header class="w3-round w3-container w3-orange w3-text-white">
                        <h6>
                            <span class="">Job: <?php echo $job->job_title; ?></span>
                        </h6>
                    </header>
                    <div class="w3-container w3-padding">
                        <p><?php echo $job->job_instructions; ?></p>
