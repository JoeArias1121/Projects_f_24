...
if (isset($_GET['jobTemplate_id']) && isset($_GET['add_task_temp_id'])) {
    ...
    $add_row_to_jobTemplateDetails_T = 'INSERT INTO f23_JobTemplateDetails_T (jobTemplate_id, taskTemplate_id, detailStatus_id) VALUES (?, ?, ?)';
...
if (isset($_GET['jobTemplate_id']) && isset($_GET['job_taskOrder'])) {
    ...
    $hide_row_from_jobTemplateDetails_T = 'UPDATE f23_JobTemplateDetails_T SET detailStatus_id = 0 WHERE jobTemplate_id = ? AND job_taskOrder = ?';
...
$jobTemplate_T_title_results = mysqli_query($db_conn, 'SELECT jobTemplate_id, job_title FROM f23_JobTemplate_T WHERE jobTemplate_id =' . $current_jobTemplate_id);

$Stmt3 = 'SELECT f23_JobTemplateDetails_T.jobTemplate_id,f23_JobTemplateDetails_T.taskTemplate_id,f23_JobTemplateDetails_T.job_taskOrder,f23_JobTemplateDetails_T.detailStatus_id, 
f23_JobTemplate_T.job_title, f23_StepTemplate_T.task_title FROM f23_JobTemplateDetails_T LEFT JOIN f23_JobTemplate_T ON f23_JobTemplateDetails_T.jobTemplate_id = f23_JobTemplate_T.jobTemplate_id 
LEFT JOIN f23_StepTemplate_T ON f23_JobTemplateDetails_T.taskTemplate_id = f23_StepTemplate_T.taskTemplate_id WHERE f23_JobTemplateDetails_T.detailStatus_id != 0 AND 
f23_JobTemplateDetails_T.jobTemplate_id =' . $current_jobTemplate_id . ' ORDER BY job_taskOrder ASC';
// view all details for a job template
$jobTemplateDetails_T_results = mysqli_query($db_conn, $Stmt3);

// task template results
$taskTemplate_T_results = mysqli_query($db_conn, 'SELECT * FROM f23_StepTemplate_T');
...
<body>
    ...
    <div class="w3-container">
        <hr>
        <table class="w3-table-all w3-round-large w3-responsive">
            <tr>
                <th>Template ID</th>
                <th>Job Template</th>
                <th>Step Template ID</th>
                <th>Step Template</th>
                <th>Step Order</th>
                <th>Remove</th>
            </tr>
        ...
        <div class="w3-bar">
            ...
            <div class="w3-bar-item">
                <div>
                    <button id="add-user-btn" class="w3-button w3-round-large w3-green">+</button>
                </div>
            </div>
            <!-- Text -->
            <div class="w3-bar-item">
                <div>
                    <p class="content">Add A Step Template</p>
                </div>
            </div>
        ...
        <div class="w3-hide" id="add-task-temp-form">
            <form action="./job-template-admin.php" method="GET">
                ...
                <label for="add_user_id">Select A Step Template ID/Title</label>
                <br>
                <div class="">
                    <div class="">
                        <select name="add_task_temp_id" class="w3-select w3-border w3-round-large w3-sand">
