...
$task_template_results = mysqli_query($db_conn, 'SELECT * FROM f23_StepTemplate_T');
else
$task_template_results = mysqli_query($db_conn, 'SELECT * FROM f23_StepTemplate_T WHERE f23_StepTemplate_T.ind_code ="'.$industryResults['ind_code'].'" OR f23_StepTemplate_T.ind_code="0"');
?>
...
<div id="workflowTemplateSearch" class="w3-card w3-white w3-round-large w3-padding w3-margin">
    ...
    <?php if ($_SESSION['user_type'] == 1) { ?>
        <button class="w3-button w3-right w3-green w3-round-large" type="button" onclick="document.getElementById('tasktemplate').style.display='block'">New Template</button><?php } ?>
        <button <?php if ($_SESSION['user_type'] == 1) echo 'style="margin-left:130px"'?> class="w3-button w3-blue w3-round-large" type="button" onclick="window.location='./dashboard.php?content=search&contentType=templates'">Job Templates</button>
        <h5>Task Template Search</h5>
    ...
    <table id="workflowTable" class="pagination w3-table-all w3-round-large w3-responsive w3-content">
        <thead>
            <tr class="w3-sand">
                ...
                <?php if ($_SESSION['user_type'] == 1) { ?>
                <th style="width:150px" class="w3-center">Users/Data</th>
                <?php }?>
            </tr>
        </thead>
        <tbody>
            ...
            <?php
            while ($template = mysqli_fetch_object($task_template_results)) {
            ?>
                <?php if ($template->templateStatus_id != 3) { ?>
                    <tr>
                        ...
                        <td class="w3-center" style="width:150px"><?php echo $template->task_title; ?></td>
                        <!-- instructions -->
                        <td><?php echo $template->task_instructions; ?></td>
                        <!-- change status to ready, not ready, or deleted -->
                        <?php if ($_SESSION['user_type'] == 1) { ?>
                            <td class="w3-center">
                                <button class="w3-button w3-blue w3-round-large" onclick="document.getElementById('admin<?php echo $template->taskTemplate_id; ?>').style.display='block'">Attach</button>
                                <div id="admin<?php echo $template->taskTemplate_id; ?>" class="w3-modal">
                                    <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:600px; margin-top:-65px;">
                                        <header class="w3-round w3-container w3-purple">
                                            ...
                                            <span onclick="document.getElementById('admin<?php echo $template->taskTemplate_id; ?>').style.display='none'" class="w3-button w3-display-topright">&times;</span>
                                        </header>
                                        <iframe height="650" width="600" style="border:none;" src="./userfunctions/jobs/template-admin.php?taskTemplate_id=<?php echo $template->taskTemplate_id; ?>"></iframe>
                                    </div>
                                </div>
                            </td>
                        <?php } ?>
...
<div id="tasktemplate" class="w3-modal">
    <div class="w3-modal-content w3-center w3-round-large w3-card-4" style="width:350px; margin-top:-100px;">
      <header class="w3-round w3-container w3-green">
        <h6 class="w3-left">Create a Task Template</h6>
        <span onclick="document.getElementById('tasktemplate').style.display='none'" class="w3-button w3-round w3-display-topright">&times;</span>
      </header>
      <iframe class="w3-margin-top" height="700" style="border:none;" src="./userfunctions/jobs/create-task-template.php"></iframe>
    </div>
  </div>
